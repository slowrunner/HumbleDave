...
MOTION_STOP    = 3
MOTION_UNKNOWN = 4
printableMOTION_STATE = ["MOTION_NONE", "MOTION_START", "MOTION_MOVING", "MOTION_STOP", "MOTION_UNKNOWN"]
...
def motionChange(egpg):
    ...
    if DEBUG: print("motor_state_l: ", motor_state_l)

    motor_state_r = egpg.get_motor_status(egpg.MOTOR_RIGHT)
    if DEBUG: print("motor_state_r: ", motor_state_r)
    ...
    if ((abs(motor_state_l[3]) < 1000) and (abs(motor_state_r[3]) < 1000)):
         ...
         # motor_state_l or _r may be -1, 0, or 1 but not be running
         if abs(motors_running) == 1:
             motors_running = 0
    else:
         motors_running = 0
    if DEBUG: print("motors_running: ", motors_running)
    if DEBUG: print("motion_state: ", motion_state)
    ...
    if DEBUG: print("curr_l_enc: {} cur_r_enc: {}".format(curr_l_enc, curr_r_enc))
    ...
    if (motion_state == MOTION_UNKNOWN):
        ...
        motion_state = MOTION_NONE
    if DEBUG: print("motionChange: returning motion_state: {}".format(printableMOTION_STATE[motion_state]))
    return motion_state
...
def main():
    ...
    # if Dave: wheelLog.info("Finished")
    time.sleep(1)
