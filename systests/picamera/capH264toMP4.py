...
# FILE: capH264toMP4.py

# Since PiCamera is not available for 64-bit systems,
# Uses raspivid to take h264 video through the pi camera
# then processes h264 video with ffmpeg to mp4 container
#
# Conversion is tolerant of interruption of raspivid capture

"""
usage: capH264toMP4.py [-h] [-t TIME] [-fps FRAMERATE] [-nr NAMEROOT]

Uses raspivid to capture video in H264 container, then uses ffmpeg to create mp4 copy
...
Outputs: <nameroot>_YYYY-MM-DD_HH-MM-SS.h264 and .h264.mp4
"""
...
def main():
    ...
    ap = argparse.ArgumentParser(
             description="Uses raspivid to capture video in H264 container, then uses ffmpeg to create mp4 copy",

             epilog="Outputs: <nameroot>_YYYY-MM-DD_HH-MM-SS.h264 and .h264.mp4")
    ...
    try:
        ...
        if (outfile != ""):
            createMP4fromH264(infile=outfile,fps=framerate)
